...
- (void) start {
    ...
    if (!self.timerRunning) {
        if (globals.gameType == kSinglePlayerGame) {
            // only for non tutorial games
            aiQueue = dispatch_queue_create( "com.d-pointer.imperium.ai", NULL );

            // we first run the AI and LOS
            [[[CCDirector sharedDirector] scheduler] scheduleSelector:@selector( runAIAndSimulation ) forTarget:self interval:sParameters[kParamEngineUpdateIntervalF].floatValue paused:NO];
        }
        else {
            // multiplayer
...
- (void) pause {
    // can only resume a single player game
    if ([Globals sharedInstance].gameType != kSinglePlayerGame) {
        CCLOG( @"only single player games can be paused!" );
        return;
    }

    if (!self.timerRunning) {
        CCLOG( @"timer not running, can not pause!" );
        return;
    }

    // can only pause a single player game
    if ([Globals sharedInstance].gameType == kSinglePlayerGame) {
        // only for non tutorial games
        // stop the AI and LOS timer
        [[[CCDirector sharedDirector] scheduler] unscheduleSelector:@selector( runAIAndSimulation ) forTarget:self];
...
- (BOOL) isPaused {
    return self.timerRunning ? NO : YES;
}


- (void) resume {
    if ([Globals sharedInstance].gameType != kSinglePlayerGame) {
        CCLOG( @"only single player games can be resumed!" );
        return;
    }

    if (self.timerRunning) {
        CCLOG( @"timer already running, can not resume!" );
        return;
    }

    CCLOG( @"resuming game" );

    // start the AI and LOS again
    [[[CCDirector sharedDirector] scheduler] scheduleSelector:@selector( runAIAndSimulation ) forTarget:self interval:sParameters[kParamEngineUpdateIntervalF].floatValue paused:NO];
    [[[CCDirector sharedDirector] scheduler] scheduleSelector:@selector( runLineOfSight ) forTarget:self interval:sParameters[kParamLosUpdateIntervalF].floatValue paused:NO];

    self.timerRunning = YES;

    // also restart the time clock
    [[Globals sharedInstance].clock start];
...
- (void) runAIAndSimulation {
    dispatch_async( aiQueue, ^(void) {
        Globals *globals = [Globals sharedInstance];

        // run the AI, but only for campaign games
        if ( globals.scenario.scenarioType == kCampaign && !sAIDisabled) {
            //CCLOG( @"==============================================================================================" );
            [globals.ai execute];
            //CCLOG( @"==============================================================================================" );
        }

        // when the AI is done we simulate the action
...
- (void) simulate {
    ...
    // check which sounds to play in a looping mode
    ...
    for (Unit *unit in globals.localUnits) {
    ...
    // check the tutorial too if we have one
    if (globals.tutorial) {
        [globals.tutorial checkTutorial];
    }

    // now finally update all icons for all units
...
- (void) checkLoopingSounds {
    // these correspond to the sound types in "LoopingSoundType"
    BOOL activeSounds[6] = {NO, NO, NO, NO, NO, NO};

    Globals *globals = [Globals sharedInstance];

    // check the missions for all units and set up the proper sound to loop
    for (Unit *unit in globals.units) {
        // only check visible and alive units
        if (unit.visible == NO || unit.destroyed) {
            continue;
        }

        switch (unit.mission.type) {
            case kMeleeMission:
                CCLOG( @"activating melee sound" );
                activeSounds[kMeleeCombat] = YES;
                break;
            ...
            case kAssaultMission:
                switch (unit.type) {
                    ...
                    case kInfantryHeadquarter:
                        activeSounds[kTroopsAssaulting] = YES;
                        break;

                    case kCavalry:
                    case kCavalryHeadquarter:
                        activeSounds[kCavalryCharge] = YES;
                        break;
            ...
            case kRoutMission:
                // TODO: routs perhaps need different sounds?
            ...
            case kChangeModeMission:
                switch (unit.type) {
                    case kInfantry:
                    case kInfantryHeadquarter:
                        activeSounds[kTroopsMarching] = YES;
                        break;

                    case kCavalry:
                    case kCavalryHeadquarter:
                        activeSounds[kCavalryMarching] = YES;
                        break;

                    case kArtillery:
                        activeSounds[kArtilleryMarching] = YES;
                        break;
                }
                break;
    ...
    // now start or stop the sounds
    for (int index = 0; index < 6; ++index) {
        if (activeSounds[index]) {
            [globals.audio startSound:index];
            //CCLOG( @"starting sound %d", index );
        }
        else {
            [globals.audio stopSound:index];
            //CCLOG( @"stopping sound %d", index );
        }
    }
}
